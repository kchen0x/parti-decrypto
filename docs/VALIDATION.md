...
- [ ] `assets/gameboard-mat.jpg`、`assets/signal-route.png` 与 `assets/secret-seal.png` 可由 `index.html` 相对加载，且无无限装饰动画或高频脚本依赖。
...
- [ ] 四人完成 2v2 分队并正常开始。
- [ ] 房主在人数不足、有人未分队、或任一队少于两人时，开始按钮仍可点击，并说明各项具体缺口；满足条件后显示可开始，Worker 仍会拒绝绕过客户端的非法 `startGame` action。
...
- [ ] 大厅“游戏规则”默认折叠；展开后完整说明开局、编码、猜测、结算、胜负与软性倒计时，且在手机窄屏不挤占分队操作。
- [ ] 在编码/猜测阶段打开性能面板：无 500ms 全量传讯 DOM 重建或循环环境动效；倒计时只更新一个文本节点，切到后台后不继续调度刷新。
